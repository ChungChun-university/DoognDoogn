...
//
//            log.info("ROLE_ADMIN과 user.getRole()이 같은가? {}", RoleType.ROLE_ADMIN.name().equals(user.getRole()));
//            // 역할에 따라 리디렉션 경로 결정
//            if (RoleType.ROLE_ADMIN.name().equals(user.getRole())) {
//                log.info("관리자로 로그인했습니다: {}", userId);
//                return "redirect:/main"; // 관리자 페이지로 리다이렉트
//            } else {
